...
hasType expr ty = runW expr `shouldSatisfy` (\ty' -> isRight (ty' >>= unify ty))
...
typecheckSpec = describe "Lambda.Core.Typecheck" $ do
  ...
  describe "w" $ do
    describe "success" $ do
      it "\\x. x : forall a. a -> a" $
        (_Abs' "x" # _Id # "x") `hasType` (_Forall' ["a"] [] # _TyFun # (_TyVar # "a", _TyVar # "a"))
      it "\\x. \\y. x : forall a b. a -> b -> a" $
        (_Abs' "x" # _Abs' "y" # _Id # "x")
          `hasType` (_Forall' ["a","b"] [] # _TyFun # (_TyVar # "a", _TyFun # (_TyVar # "b", _TyVar # "a")))
      it "rec fix f x = f (fix f) x in rec : forall a. (a -> a) -> a" $
        (_Rec # (_Binding' "fix" # _Abs' "f" # _App # (_Id # "f", _App # (_Id # "fix", _Id # "f")), _Id # "fix"))
          `hasType` (_Forall' ["a"] [] # _TyFun # (_TyFun # (_TyVar # "a", _TyVar # "a"), _TyVar # "a"))
    describe "failure" $ do
      it "\\x. x x" $
